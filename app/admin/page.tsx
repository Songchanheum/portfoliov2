...
const AdminPage = () => {
  ...
  const { data, request, isLoading } = useMutationRequest<any>({
    requestPath: "post",
      ...
      <div>
        ...
        <ul>
          {post && post?.length > 0 ? (
            post?.map((e, i) => {
              console.log(e);
              return <li key={e.title + i}>{JSON.stringify(e)}</li>;
            })
          ) : (
            <></>
          )}
        ...
        <input
          type="text"
          ref={messageRef}
          onBlur={(e) => setPosturl(e.target.value)}
          className="w-full focus:outline-none text-sm focus:placeholder-gray-400 dark:focus:placeholder-gray-100 text-gray-600 dark:text-gray-200 placeholder-gray-600 dark:placeholder-gray-200 bg-gray-200 dark:bg-gray-700 rounded-md pl-5 py-3"
        />
        <button
          onClick={() =>
